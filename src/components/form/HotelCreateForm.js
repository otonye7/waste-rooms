...
const config = {
    ...
    const HotelCreateForm = (props) => {
        const {value, setValues, handleChange, handleImageChange, handleSubmit, location, setLocation} = props;
        const {title, content, price} = value;
    return (
        <form onSubmit={handleSubmit}>
            ...
            <DatePicker
             ...
             onChange={(date, dateString) => setValues({...value, from: dateString})}
            ...
            <DatePicker
             placeholder='To date'
             className='form-control m-2'
             onChange={(date, dateString) => setValues({...value, to: dateString})}
             disabledDate={(current) => current && current.valueOf() < moment().subtract(1, 'days')}
             />
